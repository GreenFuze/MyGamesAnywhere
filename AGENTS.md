# AGENTS — MyGamesAnywhere (Codex Instructions)

Generated: 2025-12-26

This repo is intended to be implemented with **minimal interaction**. If you are an automated coding agent (e.g., Codex CLI), follow these instructions strictly.

## Source of truth
1. `docs/spec/00-index.md` (spec index)
2. `docs/spec/*` (architecture, algorithms, acceptance)
3. `contracts/*.d.ts` (typed contracts: IPC + plugin API)
4. `schemas/*.schema.json` (cloud + plugin manifest schemas)
5. `storage/sqlite-schema.sql` (SQLite DDL)
6. `tasks/*.md` (work items; execute in order)
7. Programming guidelines: **this file** (embedded) and `PROGRAMMING_GUIDELINES.md` (verbatim copy)

If there is conflict between docs:
- Prefer `contracts/` and `schemas/` over prose.
- Prefer `docs/spec/*` over older notes.
- Prefer programming guidelines below if style decisions are needed.

## Goals (MVP)
- Windows desktop app (Capacitor + Electron), codebase remains cross-platform.
- No backend server. Sync via Google Drive.
- Sources are plugins: Steam + Google Drive installers (folder-scoped scan).
- Providers are plugins: LaunchBox export, IGDB, SteamGridDB.
- Plugins are user-installable in MVP (zip install).

## Non-goals (MVP)
- No store purchasing, friends/chat, achievements, streaming, mod management.
- Do not scan entire Drive (only user-selected folders).
- Do not sync local paths/cached media.

## How to work (minimal interaction)
- Execute tasks in `tasks/` sequentially.
- After each task, ensure the **Verification** section passes.
- Keep changes small and testable; do not "big bang" multiple milestones at once.
- If an ambiguity remains after consulting specs/contracts, make the smallest reasonable choice and:
  - document it in `docs/adr/` as a short ADR
  - keep risky behavior behind a clearly named feature flag

## Commands (expected)
Provide these scripts in the repo root as implementation progresses:
- `pnpm -w typecheck`
- `pnpm -w lint`
- `pnpm -w test`
- `pnpm -w dev:desktop` (or equivalent) to run desktop app

## Definition of done (per milestone)
- All verification steps in the task file pass.
- No TypeScript `any` leakage across boundaries unless explicitly justified.
- Boundary validation is present (schemas + error envelopes).
- Errors are fail-fast with structured context.

---

## Programming guidelines (embedded)
