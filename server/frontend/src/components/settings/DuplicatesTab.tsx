...
export function DuplicatesTab() {
  ...
  const selectedDeleteableSources = useMemo(
    () => selectedSources.filter((source) => source.source.hard_delete?.eligible ?? false),
    [selectedSources],
  )
  const batchDeleteTargets = useMemo<BatchSourceDeleteTarget[]>(
    () =>
      selectedDeleteableSources.map((source) => ({
        key: sourceKey(source),
        canonicalGameId: source.canonical_game_id,
        sourceGameId: source.source.id,
        sourceLabel: sourceLabel(source),
        gameTitle: duplicateTitle(source),
      })),
    [selectedDeleteableSources],
  )
  ...
  const handleBatchDeleted = async (deletedTargets: BatchSourceDeleteTarget[], warnings: string[]) => {
    const deletedIds = new Set(deletedTargets.map((target) => target.sourceGameId))
    const deletedSources = selectedDeleteableSources.filter((source) => deletedIds.has(source.source.id))
    setSelectedIds((prev) => {
      const next = new Set(prev)
      for (const source of deletedSources) next.delete(sourceKey(source))
      return next
    })
    const warningSuffix = warnings.length
      ? ` ${warnings.length} directory cleanup warning${warnings.length === 1 ? '' : 's'}.`
      : ''
    setNotice(`Deleted ${deletedTargets.length} duplicate source record${deletedTargets.length === 1 ? '' : 's'}.${warningSuffix}`)
    setDeleteWarnings(warnings)
    await refreshAfterDelete(deletedSources)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <BatchSourceHardDeleteDialog
        ...
        title="Apply Duplicate Hard Delete"
        description="Review the backing files below before applying deletion. Google Drive files will be moved to Drive trash; other file-backed sources use their configured delete behavior."
        confirmCopy="I reviewed the listed files and want to hard delete the marked duplicate source records."
        targets={batchDeleteTargets}
