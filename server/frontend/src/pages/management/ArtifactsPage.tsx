...
export function ArtifactsPage() {
  ...
  return <div className="mga-page-enter space-y-7">
    <PageIntro eyebrow="Runtime supply" title="Emulators and runtimes" description="Emulators and runtimes MGA can hand to a connected app, with the licence and checksum that make each one safe to send." />
    <div className="grid gap-4 sm:grid-cols-3"><MetricCard label="Artifacts" value={formatCount(artifacts.length)} detail="Emulators and runtimes on file" icon={<Boxes className="h-4 w-4" />} /><MetricCard label="Emulators" value={formatCount(emulators)} detail="Ready to send to a connected app" icon={<Scale className="h-4 w-4" />} /><MetricCard label="Approved" value={formatCount(approved)} detail={`${Math.max(artifacts.length - approved, 0)} blocked or awaiting compliance evidence`} tone={approved === artifacts.length ? 'good' : 'attention'} icon={<ShieldAlert className="h-4 w-4" />} /></div>
    <SectionCard title="Artifact registry" description="MGA only sends a package when its licence allows it and its checksum matches. Anything unclear stays put.">
