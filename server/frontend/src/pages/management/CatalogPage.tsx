...
export function CatalogPage() {
  ...
  return <div className="mga-page-enter space-y-7">
    <PageIntro eyebrow="Availability intelligence" title="Catalog and versions" description="Track what is available now, what is leaving, and what changed across owned, subscription, shared, trial, cloud, and metadata-only offers." />
    <div className="grid gap-4 sm:grid-cols-3">
      <MetricCard label="Offers" value={formatCount(offers.length)} detail="Current profile-scoped catalog projection" icon={<Database className="h-4 w-4" />} />
      <MetricCard label="Leaving soon" value={formatCount(leaving)} detail="Offers with an active availability warning" tone={leaving ? 'attention' : 'good'} icon={<Clock3 className="h-4 w-4" />} />
      <MetricCard label="Stale evidence" value={formatCount(stale)} detail="Provider observations outside their freshness window" tone={stale ? 'attention' : 'good'} icon={<RefreshCw className="h-4 w-4" />} />
    </div>
    <SectionCard title="Current offer projection" description="Provider, region, delivery mode, entitlement, observed version, and freshness.">
      <QueryFeedback pending={catalog.isPending} error={catalog.error} empty={!catalog.isPending && offers.length === 0} emptyTitle="No catalog offers yet" emptyDescription="Once storefront and subscription sources synchronize, MGA will preserve their availability and version evidence here." />
