...
export function AchievementsManagementPage() {
  ...
  return <div className="mga-page-enter space-y-7">
    <PageIntro eyebrow="Achievements" title="Achievements" description="Which of your sources track achievements, and how far along each one is." />
    <div className="grid gap-4 sm:grid-cols-3"><MetricCard label="Unlocked" value={formatCount(data?.totals.unlocked_count)} detail={`Of ${formatCount(data?.totals.total_count)} across your games`} icon={<Trophy className="h-4 w-4" />} /><MetricCard label="Games with achievements" value={formatCount(systems.reduce((sum, item) => sum + item.game_count, 0))} detail={`Tracked by ${formatCount(systems.length)} of your sources`} icon={<Award className="h-4 w-4" />} /><MetricCard label="Could not update" value={formatCount(data?.refresh.failed_count)} detail={`Last success ${formatDate(data?.refresh.last_successful_at)}`} tone={(data?.refresh.failed_count ?? 0) > 0 ? 'attention' : 'good'} icon={<RefreshCw className="h-4 w-4" />} /></div>
    <SectionCard title="Sources with achievements" description="What each source has found, and when it last checked.">
      <QueryFeedback pending={query.isPending} error={query.error} empty={!query.isPending && systems.length === 0} emptyTitle="No achievement data yet" emptyDescription="Connect a source that tracks achievements, then scan it." />
