...
export function AchievementsManagementPage() {
  ...
  return <div className="mga-page-enter space-y-7">
    ...
    <div className="grid gap-4 sm:grid-cols-3"><MetricCard label="Achievements" value={formatCount(data?.totals.total_count)} detail={`${formatCount(data?.totals.unlocked_count)} unlocked`} icon={<Trophy className="h-4 w-4" />} /><MetricCard label="Games with achievements" value={formatCount(systems.reduce((sum, item) => sum + item.game_count, 0))} detail={`${formatCount(systems.length)} achievement systems`} icon={<Award className="h-4 w-4" />} /><MetricCard label="Could not update" value={formatCount(data?.refresh.failed_count)} detail={`Last success ${formatDate(data?.refresh.last_successful_at)}`} tone={(data?.refresh.failed_count ?? 0) > 0 ? 'attention' : 'good'} icon={<RefreshCw className="h-4 w-4" />} /></div>
