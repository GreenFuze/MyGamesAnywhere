...
- **`GET /api/games`** — Paginated list; each item is a full **game detail** object (same shape as `GET /api/games/{id}/detail`) so the UI can expose every metadata field via column pickers. Query: `page` (default `0`), `page_size` (default `100`, max `2000`), `sort_by` (`title`, `release_date`, `platform`, or `rating`), and `sort_dir` (`asc` or `desc`). Sorting is applied before pagination. **`page_size=0`** returns all games in one response (allowed only if the library has ≤ 20000 games). **`GET /api/stats`** includes `canonical_game_count` (same notion of “visible” canonical game).
