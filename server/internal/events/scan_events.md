...
| Event | Description |
|-------|-------------|
| `scan_started` | Scan run began. `integration_count` — integrations considered after filter. `metadata_only` indicates refresh-only mode. `integrations` contains the integration ids/labels/plugins included in this job snapshot. |
...
| `scan_metadata_started` | Metadata enrichment starting. `game_count`, `resolver_count`, `metadata_providers` (snapshot of participating metadata integrations with `integration_id`, `label`, `plugin_id`, `status`, optional `reason` / `error`). |
| `scan_metadata_phase` | Phase boundary. `phase`: `identify`, `consensus`, or `fill`. |
| `scan_metadata_plugin_started` | Resolver IPC call starting. `phase`, `plugin_id`, `metadata_integration_id`, `metadata_label`, `batch_size`. |
| `scan_metadata_game_progress` | Resolver batch progress emitted after the plugin response is processed. `phase`, `plugin_id`, `metadata_integration_id`, `metadata_label`, `game_index`, `game_count`, `game_title`. |
| `scan_metadata_plugin_complete` | Resolver IPC succeeded. `phase`, `plugin_id`, `metadata_integration_id`, `metadata_label`; identify: `matched`, `total`; fill: `filled`, `candidates`. |
| `scan_metadata_plugin_error` | Resolver IPC failed (non-fatal). `phase`, `plugin_id`, `metadata_integration_id`, `metadata_label`, `error`. |
...
- `GET /api/scan/jobs/{job_id}` keeps source integrations as the top-level rows.
- Each source row can now include `metadata_providers`, a nested list of metadata-integration participation snapshots used to restore the UI after reload or SSE reconnect.
- Metadata-provider nested statuses are limited to `pending`, `running`, `completed`, `not_used`, and `error`.
