...
func BuildRouter(b *RouteBuilder, middlewareTimeout time.Duration, spaStaticDir string) chi.Router {
	...
	r.Route("/api", func(api chi.Router) {
		retired := RetiredFeatureHandler("feature_retired")
		retiredClient := RetiredFeatureHandler("mga_client_retired")
		if b != nil {
			...
			// The compatibility window keeps legacy routes discoverable but
			// permanently removes local device/install/launch authority.
			api.Post("/devices/pair", retiredClient)
			api.Post("/devices/client-launches/redeem", retiredClient)
			api.Get("/devices/connect", retiredClient)
			api.Get("/devices/client-download", retiredClient)
			api.Get("/devices/client-installer", retiredClient)
			api.Head("/devices/client-installer", retiredClient)
			api.Get("/device-transfers/archive", retired)
			api.Head("/device-transfers/archive", retired)
			api.Get("/device-transfers/content", retired)
			api.Head("/device-transfers/content", retired)
			api.Get("/device-transfers/file", retired)
			api.Head("/device-transfers/file", retired)
			api.Get("/device-transfers/save-domain", retired)
			api.Put("/device-transfers/save-domain", retired)
			...
			api.Group(func(r chi.Router) {
				...
				registerProtectedRetiredRoutes(r, retired)
			})
			...
			api.Group(func(r chi.Router) {
				...
				if b.LegacyRetirementCtrl != nil {
					r.Get("/legacy-client-data/report", adminOnly(b.LegacyRetirementCtrl.Report))
				}
			...
			api.Group(func(r chi.Router) {
				...
				r.Get("/games/{id}/play", retired)
				r.Head("/games/{id}/play", retired)
			})
			...
			api.Get("/device-transfers/save-domain", retired)
			api.Put("/device-transfers/save-domain", retired)
			registerProtectedRetiredRoutes(api, retired)
			...
			api.Get("/games/{id}/play", retired)
			api.Head("/games/{id}/play", retired)
