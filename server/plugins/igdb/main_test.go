...
func TestLoadConfig(t *testing.T) {
	...
	os.WriteFile(cfgPath, []byte(`{"client_id":"test_id","client_secret":"test_secret"}`), 0o644)
	...
	if cfg.ClientID != "test_id" || cfg.ClientSecret != "test_secret" {
...
func loadTestConfig(t *testing.T) {
	t.Helper()
	if os.Getenv("IGDB_INTEGRATION") == "" {
		t.Skip("set IGDB_INTEGRATION=1 to run integration tests")
	}
...
func TestAuthenticate(t *testing.T) {
	...
	token, expires, err := authenticate(&auth.cfg)
	if err != nil {
	...
	t.Logf("token: %s...%s (expires %s)", token[:8], token[len(token)-4:], expires.Format("2006-01-02"))
...
func TestBuildSearchQueries(t *testing.T) {
	queries := buildSearchQueries("Donkey Kong", []int{52})
	if len(queries) != 3 {
		t.Fatalf("expected 3 queries, got %d", len(queries))
	}
	if !strings.Contains(queries[0], "platforms = 52") {
		t.Errorf("pass 1 should contain platform filter: %s", queries[0])
	}
	if strings.Contains(queries[1], "where") {
		t.Errorf("pass 2 should NOT have a where clause: %s", queries[1])
	}
	if !strings.Contains(queries[2], "name ~") {
...
func TestMultiPassSearch(t *testing.T) {
	loadTestConfig(t)
	...
	tests := []struct {
		...
		{"Donkey Kong", "arcade"},
		{"Duke Nukem 3D", "ms_dos"},
		{"Half-Life 2", "windows_pc"},
		{"Castlevania: Symphony of the Night", "ps1"},
		{"God of War", "ps2"},
	}

	for _, tc := range tests {
		igdbPlatforms := platformMap[tc.platform]
		queries := buildSearchQueries(tc.title, igdbPlatforms)
		totalResults := 0
		for i, q := range queries {
			games, err := runQuery(q)
			if err != nil {
				t.Errorf("%q pass %d: %v", tc.title, i+1, err)
				continue
			}
			t.Logf("  %q pass %d: %d results", tc.title, i+1, len(games))
			for _, g := range games {
				t.Logf("    → %q (ID: %d)", g.Name, g.ID)
			}
			totalResults += len(games)
		}
		if totalResults == 0 {
			t.Errorf("%q: no results across all passes", tc.title)
		}
	}
}
...
func TestMatchGame(t *testing.T) {
	...
	tests := []struct {
		...
		{"Super Mario Bros.", "arcade", []string{"Super Mario Bros.", "Vs. Super Mario Bros."}},
	}
...
func TestTV2GamesCoverage(t *testing.T) {
	if os.Getenv("IGDB_INTEGRATION") == "" {
		t.Skip("set IGDB_INTEGRATION=1 to run")
	}
	loadTestConfig(t)
	...
	t.Logf("\n=== IGDB TV2 Games Coverage Report ===")
