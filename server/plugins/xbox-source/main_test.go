...
func TestXboxSourcePlugin(t *testing.T) {
	...
	exePath := "./xbox-source.exe"
	if _, err := os.Stat(exePath); err != nil {
		t.Fatalf("build the plugin first: go build -o xbox-source.exe .")
	}
	...
	// 1. plugin.init (will open browser for OAuth if needed)
	t.Log("calling plugin.init (may open browser for Xbox auth)...")
	...
	t.Logf("=== Xbox Game Source Results ===")
	...
	if len(result.Games) == 0 {
		t.Error("expected at least some games from Xbox library")
	}
}
