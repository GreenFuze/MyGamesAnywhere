...
func testBinding(server, instance string) Binding {
	return Binding{ServerURL: server, WebSocketURL: "ws://" + instance + "/connect", EndpointID: "endpoint-" + instance, ClientInstanceID: instance, DisplayName: "PC / Alice"}
}
...
func TestStoreRoundTrip(t *testing.T) {
	...
	if got.LegacyLoaded || len(got.Document.Bindings) != 2 || got.Document.Bindings[1] != want.Bindings[1] {
...
func TestStoreLoadsLegacyWithoutWritingIt(t *testing.T) {
	...
	if !result.LegacyLoaded || len(result.Document.Bindings) != 1 || !result.Document.Bindings[0].LegacyIdentity {
...
func TestStoreFailsFastOnUnknownSchemaOrFields(t *testing.T) {
	...
	for _, data := range []string{
		...
		`{"schema_version":2,"bindings":[],"unknown":true}`,
