...
if (-not (Test-Path -LiteralPath $clientExe) -or -not (Test-Path -LiteralPath $agentExe) -or -not (Test-Path -LiteralPath $notices)) {
    throw "MGA Client packaging inputs are incomplete."
}
...
& $isccPath "/DAppVersion=$Version" "/DClientExe=$clientExe" "/DAgentExe=$agentExe" "/DNoticesFile=$notices" "/DOutputDir=$release" $script
