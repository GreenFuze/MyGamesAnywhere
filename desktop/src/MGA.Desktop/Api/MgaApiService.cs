...
public sealed class MgaApiService
{
    ...
    /// <summary>
    /// Creates the very first gamer profile on a freshly-set-up server via
    /// POST /api/setup/start-fresh. This endpoint requires no profile header and
    /// returns 409 if profiles already exist.
    /// </summary>
    public async Task<Profile> CreateFirstProfileAsync(
        string displayName, CancellationToken ct = default)
    {
        var body = new { display_name = displayName };
        var resp = await _http.PostAsJsonAsync("/api/setup/start-fresh", body, JsonOptions, ct)
            .ConfigureAwait(false);
        await EnsureSuccess(resp, ct).ConfigureAwait(false);
        var profile = await resp.Content.ReadFromJsonAsync<Profile>(JsonOptions, ct).ConfigureAwait(false);
        return profile ?? throw new MgaApiException(201, "Server returned null profile");
    }
